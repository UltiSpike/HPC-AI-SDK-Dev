...
| `train_mlp` | `bool` | `False` | Whether to apply LoRA adapters to MLP (Multi-Layer Perceptron) layers, including gate, up, and down projection layers. |
...
| `train_unembed` | `bool` | `False` | Whether to apply LoRA adapters to the unembedding layer (language modeling head, lm_head). |
